import { success, failure, tryCatch, all, Result } from '../src/result';
...
describe('Edge Cases Tests', () => {
  describe('Result Edge Cases', () => {
    ...
    it('should handle deeply nested Result chains', () => {
      let result: Result<unknown, number> = success(0);
  ...
  describe('Array Edge Cases', () => {
    ...
    it('should handle arrays with null/undefined values', () => {
      ...
      const mapped = map((x: any) => success(x === null ? 0 : x === undefined ? -1 : x * 2))(
        mixedArray,
      );
    ...
    it('should handle arrays with mixed types', () => {
      ...
      expect(mapped.getOrThrow()).toEqual([
        'number',
        'string',
        'object',
        'object',
    ...
    it('should handle all() with mixed Results', () => {
      const results = [success(1), success(2), failure('error'), success(3)];
  ...
  describe('Object Edge Cases', () => {
    ...
    it('should handle merge with conflicting properties', () => {
      ...
      const merged = merge((_key: string, _target: unknown, source: unknown) => success(source))(
        obj1,
        obj2,
      );
  ...
  describe('Type System Edge Cases', () => {
    it('should handle mixed error types', () => {
      ...
      expect(
        objectError.fold(
          (err: { code: number; message: string }) => err,
          () => null,
        ),
      ).toEqual({ code: 500, message: 'server error' });
      expect(
        arrayError.fold(
          (err: string[]) => err,
          () => null,
